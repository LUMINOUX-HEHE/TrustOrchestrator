---- MODULE TrustOrchestratorP6Mutant ----
\* MUTATION TEST for P6 (test plan §3): auditor escalation also injects a
\* vote (EscalateVote: raised' /\ votes' includes the auditor). Expected
\* outcome: TLC reports "Invariant P6EscalationOnly is violated" — proving
\* auditor actions cannot touch recovery votes.
...
\* MUTANT: EscalateVote — escalation also counts as a recovery vote. Votes
\* leave the council, so P6EscalationOnly must fail.
Escalate(a) ==
    /\ a \in Auditors
    /\ a \notin raised
    /\ raised' = raised \cup {a}
    /\ votes' = votes \cup {a}
    /\ UNCHANGED <<validA, validB, everRevokedA, everRevokedB, epoch, canon, anchors, alarm>>
